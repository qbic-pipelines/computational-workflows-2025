...
if not any("condition_control_treated.deseq2.results.tsv".endswith(ext) for ext in [".csv", ".tsv", ".txt"]):
    exit("Please provide a .csv, .tsv or .txt file!")

table = pd.read_csv("condition_control_treated.deseq2.results.tsv", sep=("," if "condition_control_treated.deseq2.results.tsv".endswith(".csv") else "	"), header=0)
...
table.to_csv(path.splitext(path.basename("condition_control_treated.deseq2.results.tsv"))[0]+"_filtered.tsv", sep="	", index=False)
